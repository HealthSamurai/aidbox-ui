				...
				<button
					...
					className={`px-3 py-1 rounded text-sm font-medium transition-colors ${
						mode === "json"
							...
							: "bg-background hover:bg-background-tertiary text-text-primary"
					}`}
...
export const ResourceEditorPage: React.FC<ResourceEditorPageProps> = ({
	id,
}) => {
	...
	const handleFormat = () => {
		try {
			const parsed =
				mode === "yaml" ? yaml.load(editorValue) : JSON.parse(editorValue);
	...
	const handleSave = async () => {
		try {
			setIsSaving(true);
			const parsed =
				mode === "json" ? JSON.parse(editorValue) : yaml.load(editorValue);
