...
function HumanizedValue({ children, tooltip }: HumanizedValueProps): React.ReactNode {
...
export function humanizeTelecom(value: Telecom | null | undefined): string | null {
...
export function humanizeLanguage(value: Language | null | undefined): string | null {
	if (!value) return null;
...
export function humanizeContact(value: Contact | null | undefined): string | null {
	if (!value) return null;
...
function humanizeAnnotation(value: any): string {
	...
	return [text, author ? `— ${author}` : null, time ? `(${time})` : null].filter(Boolean).join(" ");
}
...
function humanizeDataRequirement(value: any): string {
	...
	return [type, profile?.[0], mustSupport ? mustSupport.join(", ") : null].filter(Boolean).join(" ");
}
...
function humanizeSampledData(value: any): string {
	...
	return [dimensions ? `${dimensions}D` : null, period].filter(Boolean).join(" ");
}
...
function humanizeSignature(value: any): string {
	...
	return [who ? humanizeValue_(null, who, "Reference") : null, when ? `(${humanizeDatetime(when)})` : null]
		.filter(Boolean)
		.join(" ");
}
...
function humanizeTiming(value: any): string | null {
	...
	if (repeat) {
		const { frequency, period, periodUnit, boundsDuration, boundsRange, boundsPeriod, count, duration, durationUnit } =
			repeat;
		...
		if (boundsDuration) parts.push(humanizeValue_(null, boundsDuration, "Duration"));
...
function humanizeUsageContext(value: any): string {
	...
	else if (valueQuantity) parts.push(humanizeValue_(null, valueQuantity, "Quantity"));
...
function ReferenceLink({ href, onClick, children }: { href: string; onClick: () => void; children: React.ReactNode }) {
	return (
		<Link
			to={href}
			onClick={(e) => {
...
function humanizeValue_(
	...
	try {
		...
		if (["decimal", "integer", "integer64", "positiveInt", "unsignedInt", "time", "boolean"].includes(datatype))
			return String(value);

		if (key === "lastUpdated" || datatype === "instant" || datatype === "dateTime") return humanizeDatetime(value);
		...
		if (datatype === "Reference") {
			...
			if (identifier) {
				const rt = value.type || refer || (key ? key.charAt(0).toUpperCase() + key.slice(1) : "");
				const humanizedValue = value.display || `${rt}?identifier=${identifier}`;
				const navigate = useNavigate();
		...
		if (datatype === "BackboneElement" && value[key as any]?.reference) {
			...
			return (
				<HumanizedValue tooltip={JSON.stringify(valueRef.display || ref, null, " ")}>
		...
		if (datatype === "Address") {
			const humanized = Array.isArray(value) ? value.map(humanizeAddress).join(", ") : humanizeAddress(value);
		...
		if (datatype === "HumanName")
			return Array.isArray(value) ? value.map(humanizeName).join(", ") : humanizeName(value);

		if (["Quantity", "Age", "Distance", "Duration", "Count", "MoneyQuantity", "SimpleQuantity"].includes(datatype)) {
			const parts: string[] = [];
		...
		if (datatype === "Coding") return value.display || `${value.system}|${value.code}`;
		...
		if (datatype === "CodeableConcept") {
			...
			if (value.coding) {
				const displays = value.coding.map((c: any) => c.display || c.code).filter(Boolean);
		...
		if (datatype === "ParameterDefinition") return humanizeParameterDefinition(value);
...
export function humanizeValue(key: string, value: any, snapshot: Snapshot): string | React.ReactNode | null {
	const element = snapshot[key];
	const datatype = element?.datatype || element?.type;

	if (Array.isArray(value)) {
		const humanized = value.map((v) => humanizeValue_(key, v, datatype || "", element));

		if (humanized.length === 1) return humanized[0];

		if (humanized.every((h) => typeof h === "string")) return humanized.filter((h) => h && h.trim()).join(", ");
